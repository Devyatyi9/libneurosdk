...
static void on_message(ws_t *ws, const char *data, size_t len, int binary, void *userdata) {
  ...
  if (rc != 0) fprintf(stderr, "[fuzzing] send failed\n");
}
...
static const char *default_url = "ws://127.0.0.1:9001";
...
int main(int argc, char *argv[]) {
  ...
  const char *base = argc > 2 ? argv[2] : default_url;
  char url[512];
  snprintf(url, sizeof(url), "%s/runCase?case=%s&agent=ws_client", base, argv[1]);
