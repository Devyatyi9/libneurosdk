...
int main(void) {
	...
	test("no path gets slash",
	     parse_url("ws://localhost", &host, &port, &path) == 0 && port == 80 &&
	         strcmp(path, "/") == 0);
	if (host)
		free(host);
	if (path)
	...
	test("empty host", parse_url("ws:///path", &host, &port, &path) != 0);
	test("host NULL after empty host", host == NULL);
	test("path NULL after empty host", path == NULL);

	test("port zero", parse_url("ws://host:0/path", &host, &port, &path) != 0);
	...
	test("long host",
	     parse_url("ws://"
	               ...
	               "123456789012345678901234567890123456789:8080/x",
	               &host, &port, &path) == 0 &&
