...
static void on_message(ws_t *ws,
	...
	if (binary || len != strlen(expected_message) ||
		...
		fprintf(stderr, "unexpected proxy echo payload\n");
		payload_ok = 0;
	}
}
...
int main(int argc, char *argv[]) {
	...
	/* Send a message */
	printf("[send] %s\n", expected_message);
	if (ws_send(ws, expected_message, strlen(expected_message)) != 0) {
	...
	if (fail) {
		...
		printf("[pass] Proxy echo and clean Close completed successfully\n");
	}
